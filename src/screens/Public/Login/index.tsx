...
export default function Login({ navigation: { navigate } }: LoginProps) {
  ...
  const { formErrors, formData: { username, password }, validateForm, setUsername, setPassword } = useLoginValidation()
  const [login, { isLoading }] = useLoginMutation();
  const [registerBiometrics] = useRegisterBiometricsMutation();

  const customer = appState?.customer;

  const [showRegisterDeviceModal, setShowRegisterDeviceModal] = useState<boolean>(false);
  const [biometricAvailable, setIsBiometricAvailable] = useState<boolean>(false);

  const prepUserDetails = async ({ status, message, data }: AuthResponse<LoginResponse>) => {
    const rnBiometrics = new ReactNativeBiometrics();

    const { keysExist } = await rnBiometrics.biometricKeysExist()
    !keysExist && await handleRegisterBiometrics()
    await persistAppState({ customer: { ...data.customer, username: !!username ? username : customer?.username } })
    dispatch(setCustomer({ ...data.customer, username: !!username ? username : customer?.username }))
    showToast(status ? 'success' : 'danger', message);
  }

  const handleLogin = async (loginType: 'password' | 'biometrics', signature?: string, payload?: string) => {
    ...
    try {
      ...
      showToast('danger', error.data?.message || error.message || DEFAULT_ERROR_MESSAGE, 6000);
    }
  };
  ...
  const handleRegisterBiometrics = async () => {
    ...
    try {
      ...
      if (status) {
        persistAppState({ enableBiometrics: true, customer: { ...customer, username } });
        dispatch(updateAppstate({ enableBiometrics: true, customer: { ...customer, username } }));
        return;
      }
  ...
  const handleBiometricLogin = async () => {
    ...
    try {
      // Payload for signing
      const payload = `Login attempt for user ${customer?.username || username} at ${new Date().toISOString()}`;
    ...
    <AuthLayout isLoading={isLoading}>
      ...
      <View style={styles.overlay}>
        <View style={styles.overlayContent}>
          <Typography
            type="heading5-sb"
            title={`${customer?.username ? `Welcome back, ${customer.first_name || "Customer"}` : 'Login with your username and password'}`}
          />
          <View style={styles.inputGroup}>
            {!customer?.username ? <TextInput
              type="phone"
              label="Username"
          ...
          <Button title="Login" onPress={() => validateForm(() => handleLogin('password'))}></Button>
          ...
          <View>
            {biometricAvailable ?
              (
                <TouchableOpacity style={styles.biometricsContainer} onPress={handleBiometricLogin}>
                  <Image
