...
type CableTVSubscriptionScreenNavigationProp = StackNavigationProp<WalletStackParamList, 'ConfirmTransactionScreen'>;

interface ICableTVPlan {
    name: string;
    code: string;
    amount: string;
}

interface ICableTVSubscriptionProps {
    navigation: CableTVSubscriptionScreenNavigationProp;
    route: RouteProp<WalletStackParamList, 'CableTVSubscriptionScreen'>;
}

interface IPlansMap{
    DSTV: ICableTVPlan[];
    GoTV: ICableTVPlan[];
    Startimes: ICableTVPlan[];
    TSTV: ICableTVPlan[];
}

const plans: ICableTVPlan[] = [
    {
    ...
    {
        name: 'CableTV Confam Streaming Subscription',
        code: '2',
        amount: '1,200'
    }
]

const DSTVPlans: ICableTVPlan[] = [
    {
...
const StartimesPlans: ICableTVPlan[] = [
    {
...
export default function CableTVSubcription({ navigation, route }: ICableTVSubscriptionProps) {
    const [showProductModal, setShowProductModal] = useState<boolean>(false);
    const [selectedPlan, setSelectedPlan] = useState<ICableTVPlan>({ name: '', code: '', amount: '' })
                ...
                <View style={mainWalletStyles.inputGroup}>
                    {!showBeneficiaryCard && (
                        <View style={mainWalletStyles.beneficiaryCardContainer}>
                            <TextInput placeholder={`${billProvider} Smart Card Number`} />
